...
public class BoatRace {
    ...
    private static Item[] riverList = river.getRiverList();
        ...
        out: while (player1.getLocation() != 100 && player2.getLocation() != 100) {
            ...
            if ((player1.getLocation() < 100)) {
                ...
                while (!(riverList[player1.getLocation()] instanceof Blank)) {
                    encounterItem(riverList[player1.getLocation()], player1);
                    if ((player1.getLocation() >= 100)) {
                        winner = player1;
                        break out;
            ...
            if ((player2.getLocation() < 100)) {
                ...
                while (!(riverList[player2.getLocation()] instanceof Blank)) {
                    encounterItem(riverList[player2.getLocation()], player2);
                    if ((player2.getLocation() >= 100)) {
                        winner = player2;
                        break out;
    ...
    private static void encounterItem(Item item, Player player) {
        if (item instanceof Current) {
            ((Current) item).generateMagnitude();
            System.out.print(item);
            player.setLocation(player.getLocation() + item.getMagnitude());

        } else if (item instanceof Trap) {
            ((Trap) item).generateMagnitude();
            System.out.print(item);
            player.setLocation(player.getLocation() - item.getMagnitude());
